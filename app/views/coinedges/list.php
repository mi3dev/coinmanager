...
function sortLink($label,$col,$curS,$curD){
  ...
  $url  = Url::build('coinedges/list',['sort'=>$col,'dir'=>$next,'page'=>1]);
  $c = ($curS===$col)?($curD==='ASC'?' ▲':' ▼'):'';
  return '<a href="'.$url.'" class="hover:underline">'.$label.$c.'</a>';
}
$routeForPager = $_GET['route'] ?? 'coinedges/list';
?>
<section class="space-y-4">
  <div class="flex items-center justify-between">
    <h1 class="text-lg font-semibold">Číselník: Typy hran</h1>
    <a href="<?= Url::build('coinedges/create') ?>" class="rounded-md bg-sky-500 px-3 py-2 text-sm font-semibold text-slate-950 hover:bg-sky-400">+ Nový záznam</a>
  </div>

  <div x-data="{adv:false}" class="space-y-2">
    <form method="get" class="flex gap-2">
      <input type="hidden" name="route" value="coinedges/list">
          ...
          <tr class="hover:bg-slate-800/40">
            ...
            <td class="px-3 py-2 text-right">
              <a class="rounded-md border border-slate-700 px-2 py-1 text-xs hover:bg-slate-800"
                 href="<?= Url::build('coinedges/edit',['id'=>$r['id']]) ?>">Upravit</a>
              <form class="inline-block" method="post" action="<?= Url::build('coinedges/toggle') ?>"
