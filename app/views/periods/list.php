...
<section class="space-y-4">
  ...
  <div x-data="{adv:false}" class="space-y-2">
    <form method="get" class="flex gap-2">
      ...
      <?php if (!empty($q) || !empty($showInactive)): ?>
        <a href="<?= Url::build('periods/list') ?>"
           class="flex items-center gap-1 rounded-md border border-slate-700 px-3 py-2 text-sm hover:bg-slate-800">
