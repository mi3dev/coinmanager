...
$mode = $mode ?? 'create';
$isEdit = ($mode==='edit');
$action = $isEdit ? 'index.php?route=coinmints/update' : 'index.php?route=coinmints/store';
?>
<section class="space-y-4 max-w-4xl">
  <h1 class="text-lg font-semibold"><?= $isEdit ? 'Upravit mincovnu' : 'Přidat mincovnu' ?></h1>
  <?= View::flashHtml() ?>
  ...
  <form method="post" action="<?= $action ?>" class="space-y-4" autocomplete="off">
    ...
    <div class="grid gap-4 sm:grid-cols-2">
      ...
      <label class="block">
        <span class="mb-1 block text-sm text-slate-300">Země (max 64)</span>
        <input type="text" name="country" maxlength="64"
               class="w-full rounded-md border border-slate-700 bg-slate-900 px-3 py-2 text-sm"
               value="<?= htmlspecialchars($item['country'] ?? '') ?>">
        <?php if (!empty($errors['country'])): ?><div class="mt-1 text-xs text-red-300"><?= htmlspecialchars($errors['country']) ?></div><?php endif; ?>
      </label>
    </div>

    <div class="grid gap-4 sm:grid-cols-2">
      <label class="block">
        <span class="mb-1 block text-sm text-slate-300">Značka (max 10)</span>
        <input type="text" name="mark" maxlength="10"
               inputmode="latin" pattern="^[A-Za-z0-9.\-\*]+$" placeholder="např. B, KB, *"
               class="w-full rounded-md border border-slate-700 bg-slate-900 px-3 py-2 text-sm"
               value="<?= htmlspecialchars($item['mark'] ?? '') ?>">
        <?php if (!empty($errors['mark'])): ?><div class="mt-1 text-xs text-red-300"><?= htmlspecialchars($errors['mark']) ?></div><?php endif; ?>
      </label>
    ...
    <div class="flex justify-end gap-2 pt-2">
      <a href="<?= Url::build('coinmints/list') ?>" class="rounded-md border border-slate-700 px-3 py-2 text-sm hover:bg-slate-800">Zpět</a>
