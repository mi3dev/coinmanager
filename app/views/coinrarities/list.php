...
function sortLink($label,$col,$curS,$curD){
  ...
  $url  = Url::build('coinrarities/list',['sort'=>$col,'dir'=>$next,'page'=>1]);
  $c = ($curS===$col)?($curD==='ASC'?' ▲':' ▼'):'';
  return '<a href="'.$url.'" class="hover:underline">'.$label.$c.'</a>';
}
$routeForPager = $_GET['route'] ?? 'coinrarities/list';
?>
<section class="space-y-4">
  <div class="flex items-center justify-between">
    <h1 class="text-lg font-semibold">Číselník: Vzácnosti</h1>
    <a href="<?= Url::build('coinrarities/create') ?>" class="rounded-md bg-sky-500 px-3 py-2 text-sm font-semibold text-slate-950 hover:bg-sky-400">+ Nová vzácnost</a>
  </div>

  <div x-data="{adv:false}" class="space-y-2">
    <form method="get" class="flex gap-2">
      <input type="hidden" name="route" value="coinrarities/list">
      <input class="w-full rounded-md border border-slate-700 bg-slate-900 px-3 py-2 text-sm"
             type="text" name="q" placeholder="Hledat v názvu, zobrazení, popisu, poznámce…"
    ...
    <table class="min-w-full text-sm">
      <thead class="bg-slate-900 text-slate-400">
        <tr>
          ...
          <th class="px-3 py-2 text-left"><?= sortLink('Úroveň','level',$sort,$dir) ?></th>
      ...
      <tbody class="divide-y divide-slate-800">
        <?php foreach ($rows as $r): ?>
          <tr class="hover:bg-slate-800/40">
            ...
            <td class="px-3 py-2"><?= (int)$r['level'] ?></td>
            ...
            <td class="px-3 py-2 text-right">
              <a class="rounded-md border border-slate-700 px-2 py-1 text-xs hover:bg-slate-800"
                 href="<?= Url::build('coinrarities/edit',['id'=>$r['id']]) ?>">Upravit</a>
              <form class="inline-block" method="post" action="<?= Url::build('coinrarities/toggle') ?>"
