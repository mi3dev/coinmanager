...
$mode = $mode ?? 'create';
$isEdit = ($mode==='edit');
$action = $isEdit ? 'index.php?route=coinmetals/update' : 'index.php?route=coinmetals/store';
?>
<section class="space-y-4 max-w-4xl">
  <h1 class="text-lg font-semibold"><?= $isEdit ? 'Upravit kov/slitinu' : 'Přidat kov/slitinu' ?></h1>
  <?= View::flashHtml() ?>
  ...
  <form method="post" action="<?= $action ?>" class="space-y-4" autocomplete="off">
    ...
    <div class="flex justify-end gap-2 pt-2">
      <a href="<?= Url::build('coinmetals/list') ?>" class="rounded-md border border-slate-700 px-3 py-2 text-sm hover:bg-slate-800">Zpět</a>
