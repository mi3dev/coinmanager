...
$mode = $mode ?? 'create';
$isEdit = ($mode==='edit');
$action = $isEdit ? 'index.php?route=coinrarities/update' : 'index.php?route=coinrarities/store';
?>
<section class="space-y-4 max-w-4xl">
  <h1 class="text-lg font-semibold"><?= $isEdit ? 'Upravit vzácnost' : 'Přidat vzácnost' ?></h1>
  <?= View::flashHtml() ?>
  ...
  <form method="post" action="<?= $action ?>" class="space-y-4" autocomplete="off">
    ...
    <div class="grid gap-4 sm:grid-cols-2">
      ...
      <label class="block">
        <span class="mb-1 block text-sm text-slate-300">Úroveň (1 = běžná … 5 = unikát)</span>
        <input type="number" name="level" inputmode="numeric" min="1" max="9" step="1"
               class="w-full rounded-md border border-slate-700 bg-slate-900 px-3 py-2 text-sm"
               value="<?= htmlspecialchars((string)($item['level'] ?? '1')) ?>">
        <?php if (!empty($errors['level'])): ?><div class="mt-1 text-xs text-red-300"><?= htmlspecialchars($errors['level']) ?></div><?php endif; ?>
      </label>
    </div>
    ...
    <div class="flex justify-end gap-2 pt-2">
      <a href="<?= Url::build('coinrarities/list') ?>" class="rounded-md border border-slate-700 px-3 py-2 text-sm hover:bg-slate-800">Zpět</a>
